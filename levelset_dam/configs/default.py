...
def get_config():
    ...
    config.mode = "eval_s2s"   # "train" or "eval" or "train_eval" or "train_eval_s2s"
    config.dataset = "dam_nx41_nt160_t0.80_20_2.npy"
    # config.dataset = "dam_nx41_nt100.npy"
    config.rho1 = 1000          # Density (kg/m3) | Water
    config.rho2 = 50             # Density (kg/m3) | Air
    ...
    wandb.name = "default"
    wandb.tag = []
    wandb.notes = "s2s scale 2, 20_2 ds, t0.8, s2s=16, 20k, bs=4096, pi init"
    ...
    nondim.rho_ratio = 50 / 1000             # rho_air/rho_water
    ...
    training.max_steps = 20 # 20000
    training.batch_size_per_device = 32 # 4096
    training.s2s = True                # Sequence to sequence learning
    training.num_time_windows = 2    # For seq2seq
    training.first_s2s_steps_factor = 2 # Factor to increase steps for first time window
    training.s2s_transfer_weights = True # Transfer grad norm weights to subsequent time windows
    training.grad_clip = 2.0   # Non dimensional gradient clipping min/max
    training.re_schedule = None   # Increase re during training according to a schedule "step", "sigmoid" or None
    training.re_schedule_n = 20      # Number of steps in the step re schedule
