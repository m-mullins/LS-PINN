...
def get_config():
    ...
    config.mu1 = 0.04   # Dynamic viscosity (Pa.s) | Water
    config.mu2 = 0.02   # Dynamic viscosity (Pa.s) | Air
    ...
    wandb.name = "curri"
    wandb.tag = []
    wandb.notes = "fine reinit, s2s=5, 30k, bs=4096"
    ...
    nondim.Re = 1000 * ((9.81 * 0.584) ** 0.5) * 0.584 / 0.04 # Re = rho_ref * sqrt(gL) * L / mu_ref
    nondim.rho_ratio = 500 / 1000    # rho_air/rho_water
    nondim.mu_ratio = 0.02 / 0.04   # mu_air/mu_water

    # Physics-informed initialization
    config.use_pi_init = False
    config.pi_init_type = None   # "initial_condition"
    ...
    transfer.datasets = ["dam_nx41_nt50_t0.25_2_2.npy", "dam_nx41_nt50_t0.25_10_2.npy", "dam_nx41_nt50_t0.25_20_2.npy", "dam_nx41_nt50_t0.25_20_2.npy", "dam_nx41_nt50_t0.25_100_2.npy", "dam_nx41_nt50_t0.25_200_2.npy", "dam_nx41_nt50_t0.25_200_2.npy", "dam_nx41_nt50_t0.25.npy"]    # List of dataset filenames for curriculum training
    transfer.iterations = [5000, 5000, 5000, 5000, 5000, 5000, 5000, 40000]  # List of training iterations for each dataset
    ...
    transfer.rho1s = [1000., 1000., 1000., 1000., 1000., 998.]  # List of densities for each dataset
    transfer.rho2s = [500., 100., 50., 25., 10., 5., 2.5, 1.2]    # List of densities for each dataset
    transfer.mu1s = [0.04, 0.04, 0.04, 0.04, 0.04, 0.04, 0.04, 0.04]   # List of viscosities for each dataset
    transfer.mu2s = [0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02]   # List of viscosities for each dataset
    ...
    training.num_time_windows = 5    # For seq2seq
    training.grad_clip = 5.0   # Non dimensional gradient clipping min/max
