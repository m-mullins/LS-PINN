...
from jax import lax, jit, grad, pmap, random, tree_map, jacfwd, jacrev, debug
...
class PINN:
    ...
    def loss(self, params, weights, batch, step, *args):
        # Compute losses
    ...
    def compute_weights(self, params, batch, step, *args):
        if self.config.weighting.scheme == "grad_norm":
            ...
            if 'eik_p' in self.config.weighting.init_weights:
                w["eik_p"] = jnp.array(self.config.weighting.init_weights["eik_p"]) # Keep eik_p constant
            # if 'mass_p' in self.config.weighting.init_weights:
            #     w["mass_p"] = jnp.array(self.config.weighting.init_weights["mass_p"]) # Keep mass_p constant
