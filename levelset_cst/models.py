...
class LevelSet(ForwardIVP):
    def __init__(self, config, p0, t_star, x_star, y_star, time_offset=None):
        ...
        self.exact_mass = jnp.pi * 0.15**2 # Area of circle
        # self.bc_coords = bc_coords
        # self.bc_values = bc_values
        # self.vx = vx  # Known velocity field in x-direction
        # self.vy = vy  # Known velocity field in y-direction
    ...
    # def f_vx(self, t, x ,y):    # constant
    #     vx = jnp.where(t < 0.5, 0.2, -0.2)
    #     return vx
    
    # def f_vy(self, t, x ,y):    # constant
    #     return 0.0

    # def f_vx(self, t, x ,y):    # linear
    ...
    #     vx = jnp.where(t < (self.config.T/2), t/(self.config.T/2), -((self.config.T)-t)/(self.config.T/2)) * self.config.vx
    #     return vx
    
    # def f_vy(self, t, x ,y):    # linear
    #     if self.config.nondim.nondimensionalize == True:
    #         t = t * self.config.nondim.T_star
    #         x = x * self.config.nondim.X_star
    #         y = y * self.config.nondim.Y_star
    #     vy = jnp.where(t < (self.config.T/2), t/(self.config.T/2), -((self.config.T)-t)/(self.config.T/2)) * self.config.vy
    #     return vy

    def f_vx(self, t, x, y):    # vortex
        if self.config.nondim.nondimensionalize == True:
            t = t * self.config.nondim.T_star
            x = x * self.config.nondim.X_star
            y = y * self.config.nondim.Y_star

        vx = -2.0 * jnp.square(jnp.sin(jnp.pi * x)) * jnp.cos(jnp.pi * y) * jnp.sin(jnp.pi * y) * jnp.cos(jnp.pi * t / self.config.T)

        if self.config.nondim.nondimensionalize == True:
            vx = vx / self.config.nondim.V_star
        return vx

    def f_vy(self, t, x, y):    # vortex
        if self.config.nondim.nondimensionalize == True:
            t = t * self.config.nondim.T_star
            x = x * self.config.nondim.X_star
            y = y * self.config.nondim.Y_star

        vy = 2.0 * jnp.square(jnp.sin(jnp.pi * y)) * jnp.cos(jnp.pi * x) * jnp.sin(jnp.pi *x) * jnp.cos(jnp.pi * t / self.config.T)
