...
def get_dataset(file_path):
    ...
    phi_ref = data['phi']  # Level-set function over time [t,x,y]
    # velocity_data = data['velocity_data']    # Velocity field over time (vx, vy)
    # temporal_coords = data['temporal_coords']   # Velocity field over time with coordinates [nt,nx*ny,t/x/y/vx/vy]
    # vx = velocity_data[:,:,:,0]
    # vy = velocity_data[:,:,:,1]
    x_star = data['x']  # x grid points
    y_star = data['y']  # y grid points
    t_star = data['t']  # time points
    fem_mass_ape = data['fem_mass_ape']
    fem_mass_mape = fem_mass_ape.mean()
    return phi_ref, t_star, x_star, y_star, fem_mass_ape
    # return phi_ref, t_star, x_star, y_star, temporal_coords
