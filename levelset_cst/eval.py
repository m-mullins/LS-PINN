...
def evaluate_s2s(config: ml_collections.ConfigDict, workdir: str):
    # p_ref, t_star, x_star, y_star, vx, vy = get_dataset(os.path.join("data",config.dataset))
    ...
    for idx in range(config.training.num_time_windows):
        ...
        if config.multi == True:
            model = models.LevelSetMulti(config, p0, t, x_star, y_star)
    ...
    if config.nondim.nondimensionalize == True:
        ...
        # p_star = p_star * jnp.std(p_ref) + jnp.mean(p_ref)
        # p_pred = p_pred * jnp.std(p_ref) + jnp.mean(p_ref)
    ...
    file_name = f'level_set_vortex.gif'
    ...
    wandb.log({f"Level set vortex fields": wandb.Video(os.path.join(save_dir, file_name))})
...
def plot_sliced_results(config: ml_collections.ConfigDict, workdir: str):
    ...
    if config.training.num_time_windows > 1:
        p_ref = p_ref[:-1, :]   # Remove last time step
    p0 = p_ref[0, :]
    ...
    for idx in range(config.training.num_time_windows):
        # Get the reference solution for the current time window
        p = p_ref[num_time_steps * idx : num_time_steps * (idx + 1), :]
    ...
    max_abs_error = jnp.max(jnp.abs(p_ref - p_pred))
    min_p = min(jnp.min(p_pred),jnp.min(p_ref))
    max_p = max(jnp.max(p_pred),jnp.max(p_ref))
    ...
    for idx in range(len(ts)):
        ...
        pcm1 = axes[idx, 0].pcolormesh(X, Y, p_ref[ts[idx]], cmap='jet', shading='gouraud', vmin=min_p, vmax=max_p)
        pcm2 = axes[idx, 1].pcolormesh(X, Y, p_pred[ts[idx]], cmap='jet', shading='gouraud', vmin=min_p, vmax=max_p)
        pcm3 = axes[idx, 2].pcolormesh(X, Y, jnp.abs(p_ref[ts[idx]] - p_pred[ts[idx]]), cmap='jet', shading='gouraud', vmin=0, vmax=max_abs_error)
    ...
    # Set up the figure and axes
    fig, axes = plt.subplots(1, len(ts), figsize=(6*len(ts), 5))

    for idx in range(len(ts)):
        # Row (p: Ground truth, Prediction, Absolute Error)
        # pcm1 = axes[idx, 0].pcolormesh(X, Y, p_ref[ts[idx]], cmap='jet', shading='gouraud', vmin=min_p, vmax=max_p)
        pcm2 = axes[idx].pcolormesh(X, Y, p_pred[ts[idx]], cmap='jet', shading='gouraud', vmin=min_p, vmax=max_p)
        # pcm3 = axes[idx, 2].pcolormesh(X, Y, jnp.abs(p_ref[ts[idx]] - p_pred[ts[idx]]), cmap='jet', shading='gouraud', vmin=0, vmax=max_abs_error)
        # fig.colorbar(pcm1, ax=axes[idx, 0])
        fig.colorbar(pcm2, ax=axes[idx])
        # fig.colorbar(pcm3, ax=axes[idx, 2])

        # Titles for p
        # axes[idx, 0].set_title(f'Reference p t = {round(ts[idx]*dt,2)} s')
        axes[idx].set_title(f'Predicted p t = {round(ts[idx]*dt,2)} s')
        # axes[idx, 2].set_title(f'Absolute Error p t = {round(ts[idx]*dt,2)} s')
    ...
    # Save the animation as a GIF
    file_name = f'levelset_p_ts_pred.png'
    plt.savefig(os.path.join(save_dir, file_name))
...
def plot_mass_loss(config: ml_collections.ConfigDict, workdir: str):
    ...
    # Calculate mass ref
    negative_counts_ref = jnp.sum(p_ref < 0, axis=(1, 2))
