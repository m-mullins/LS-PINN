...
def get_config():
    ...
    config.mode = "train_eval_multi"   # "train" or "eval" or "train_eval" or "train_eval_s2s"
    # config.dataset = "vortex_nx101_nt81_T8.npy"
    config.dataset = "vortex_nx21_nt81_T8.npy"
    config.multi = True     # Add second level for refinement of eikonal or mass terms
    config.T = None  # Leave as none, updated in train script
    
    # Weights & Biases
    config.wandb = wandb = ml_collections.ConfigDict()
    wandb.project = "PINN-Level-set"
    wandb.name = "pirate_multi_vortex"
    wandb.tag = ["multi"]
    wandb.notes = "XFEM ds"
    ...
    nondim.P_star = 1
    nondim.V_star = 1
    nondim.T_star = 4.0 # 4.0 for vortex T=8 and tw=2
    ...
    weighting.causal_tol = 1.5  # epsilon
    ...
    training1.max_steps0 = None
    training1.batch_size_per_device = 64 # 128
